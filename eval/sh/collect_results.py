...
def process_file(args):
    ...
    try:
        ...
        return model_name_norm, benchmark, {"acc": acc, "tokens": avg_tokens}
...
def sync_to_wandb(args, results, project_name, df, plot_dir, csv_path):
    ...
    run = wandb.init(project=project_name, name=args.wandb_run_name, reinit=True)
...
def main(args):
    ...
    if args.use_wandb:
        ...
        sync_to_wandb(
            args, results, args.wandb_project, df, args.plot_dir, args.output_path
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base_dir",
        type=str,
        default="/data/simpleRL-reason/eval/Qwen2.5-Math-7B-Instruct-Math-Eval",
    )
    parser.add_argument(
        "--model_name",
        type=str,
        default="/data/simpleRL-reason/train/checkpoints/Qwen2.5-Math-7B_ppo_from_base_math_lv35_quicker_experiment",
    )
    ...
    parser.add_argument(
        "--wandb_api_key", type=str, default="5fb2c3eb35cb3bc0124a02069ce91eedc6570e5a"
    )
